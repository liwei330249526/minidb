 ...
 */
class ArrGetLogicalOperator : public LogicalOperator
{
public:
    ArrGetLogicalOperator(const std::vector<Value>& values, ReadWriteMode mode);
    virtual ~ArrGetLogicalOperator() = default;

    LogicalOperatorType type() const override { return LogicalOperatorType::ARR_GET; }

    const std::vector<Value>& getArr() const { return values_; }
